...
int main() {
    ...
    // step 6: test insertOrUpdate

    Fraction
            f1(22,7),       // a couple of test values
    f2(355,113);

    // add the key. it should not be there since all other keys are lower case.
    // the dictionary should have room since we just deleted 50 elements.
    okay = true;
    try {
        dict.insertOrUpdate("ANYKEY",f1);
    } catch (overflow_error &e) {
        cout << "Error: " << e.what() << endl;
        okay = false;
    } catch (...) {
    ...
    cout << "    insertOrUpdate: " << OPF(okay) << endl;
    ...
    // getFirstKey returns false if no key exists
    if (dict.getFirstKey(key)) {
        cout << "getFirstKey reports remaining keys after clear()" << endl;
        okay = false;
    }
